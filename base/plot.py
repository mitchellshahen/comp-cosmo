...
def luminosity_contributions_plot(stellar_structure, radius, density, temperature):
    """
    ...
    epsilon_pp = stellar_structure.pp_chain_energy(rho=density, T=temperature)

    # get the energy generation rate for the CNO cycle
    epsilon_cno = stellar_structure.cno_cycle_energy(rho=density, T=temperature, X_cno=None)

    # calculate the luminosity gradient using only the PP-chain
    pp_chain_lumin_grad = stellar_structure.energy_equation(
        r=radius,
        rho=density,
        T=temperature,
    ...
    cno_cycle_lumin_grad = stellar_structure.energy_equation(
        r=radius,
        rho=density,
        T=temperature,
    ...
    total_lumin_grad = stellar_structure.energy_equation(
        r=radius,
        rho=density,
        T=temperature,
        energy_gen=None
...
def opacity_contributions_plot(stellar_structure, radius, density, temperature):
    """
    ...
    for i, radius_value in enumerate(radius):
        ...
        kappa_es = stellar_structure.opacity_e_scatter()
        kappa_ff = stellar_structure.opacity_ff_scatter(rho=density[i], T=temperature[i])
        kappa_hm = stellar_structure.opacity_hm_scatter(rho=density[i], T=temperature[i])
        mean_opacity = stellar_structure.mean_opacity(rho=density[i], T=temperature[i])
    ...
    # set the ylabel
    plt.ylabel(r'Logarithm of Opacity ($\text{log}_{10}(\kappa)$)')
